...
class Sql:
    ...
    def get_base_sql(self):
        ...
        if_sql_temple = "if(case when a.{} is null and b.{} is null then false else nvl(a.{} != b.{}, True) end,concat_ws('&#','a_{}',nvl(string(a.{}),'值为null'),string(b.{})),null)"
        casewhen_sql_temple = "case when a.{} is null and b.{} is null then false else nvl(a.{} != b.{}, True) end"
        for i in range(len(table1)):
            list1.append('a.' + table1[i] + ' as a_{}'.format(table1[i]))
            list2.append('b.' + table2[i] + ' as b_{}'.format(table2[i]))
            list3.append(if_sql_temple.format(table1[i], table2[i], table1[i], table2[i], table1[i], table1[i], table2[i]))
            list4.append(casewhen_sql_temple.format(table1[i], table2[i], table1[i], table2[i]))
    ...
    def get_sql3(self):
        base_sql = self.get_base_sql()
        s = self.get_sql2()
        sql3 = 'select ' + "split(single_diff,'&#')[0] as single_diff,split(single_diff,'&#')[1] as a_single_detail,split(single_diff,'&#')[2] as b_single_detail," + \
               "{},row_number() over (partition by split(single_diff,'&#')[0]) rowNum from ({}) t".format(
            ','.join(base_sql[4]), s)
        return sql3
    ...
    def get_sql4(self):
        base_sql = self.get_base_sql()
        s = self.get_sql3()
        sql4 = 'select ' +"single_diff,a_single_detail,b_single_detail,"+ "{},rowNum from ({}) t where rowNum <= 50 limit 5000;".format(
            ','.join(base_sql[4]), s)
        return sql4
...
if __name__ == '__main__':
    ...
    content = 'set hive.strict.checks.type.safety=false;' + '\n\n\n' + \
              '-- 表单A和表单B的交集差异明细' + '\n' + cross_sql + '\n\n\n' + \
              '-- 表单A和表单B的交集差异数量统计' + '\n' + difference_sql + '\n\n\n' + \
              '-- 表单A存在表单B不存在明细' + '\n' + aexist_sql + '\n\n\n' + \
              '-- 表单B存在表单A不存在明细' + '\n' + bexist_sql + '\n\n'
